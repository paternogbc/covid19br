...
covid_br_states %>% filter(date == used_date) %>% 
  ...
  ungroup() %>% 
  slice(1:5) %>% 
  pull(state) -> top_state
  
covid_br_states  %>% filter(state %in% top_state) %>% 
  ggplot(aes(y = confirmed_cases + 1, x = date, color = state)) +
  geom_point(size = 5, alpha = .75) +
  geom_line() +
  scale_y_continuous(trans = "log10", limits = c(1, 300)) +
  theme_cowplot() +
  geom_point(data = covid_br_all %>% filter(confirmed_cases > 10 & date > "2020-02-22"), 
             aes(y = confirmed_cases + 1, x = date, color = "Brasil"), size = 5, alpha = .75) +
  geom_line(data = covid_br_all %>% filter(confirmed_cases > 10 & date > "2020-02-22"), 
            aes(y = confirmed_cases + 1, x = date, color = "Brasil")) +
  theme_cowplot(font_size = 20) +
