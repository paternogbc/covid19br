...
pop <- info_states
d2  <- left_join(x = covid_br_states, y = pop) %>% filter(date == used_date)

d2  %>% mutate(cases_p_100k = round((confirmed_cases / pop) * 100000, digits = 3),
               cases_p_k2  = confirmed_cases / area,
               cases_p_hab_k2 = cases_p_100k/area) -> d3

states2 <-left_join(states, d3 %>% filter(date == used_date), by = c("abbrev_state" = "state_ID"))

# Plot map
g2 <- ggplot() +
  geom_sf(data = states2, aes(fill = cases_p_100k), color= "black", size=.15) +
  theme_minimal(base_size = 16) +
  labs(
       title = paste("Número de casos confirmados por 100 mil habitantes"),
...
```{r}
d3 %>% select(Estado = state, "Casos confirmados" = confirmed_cases, "Casos por 100 mil habitantes" = cases_p_100k) %>% 
