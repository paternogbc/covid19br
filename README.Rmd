...
g1 <- ggplot() +
  ...
  labs(title = "Corona vírus no Brasil",
       ...
       caption = paste("Fonte: dados do Ministério da Saúde | atualizado", used_date)) +
  theme( 
...
g2 <- ggplot(covid_br_all, aes(y = confirmed_cases, x = date, label = confirmed_cases)) +
  ...
  annotate(geom='text', x = as.Date("2020-02-26"), y = 160, label = "Primeiro caso confirmado (26 de fevereiro)", hjust =  0, size = 7) +
  theme_cowplot(font_size = 20) +
  labs(y = "Casos confirmados (covid-19)",
       x = "Data",
       title = "Coronavirus no Brasil",
       subtitle = "O número de casos cresce exponencialmente",
       caption = "Fonte: dados do Ministério da Saúde | atualizado: 16/03/2020") +
     theme( 
